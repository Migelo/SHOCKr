...
double EquationsEuler::totalEnergy(double p, double rhoV2) { return p / (gamma - 1) + 0.5 * rhoV2; }

double EquationsEuler::getPressure(double e, double rhoV2) { return (gamma - 1) * (e - 0.5 * rhoV2); }

double EquationsEuler::getSoundSpeed(const std::vector<double>& quantities)
{
  double rhoV2 = quantities[XMOM] * quantities[XMOM] / quantities[DENS];
  double p     = getPressure(quantities[ENERGY], rhoV2);
