...
void OutputEulerPassiveScalar::makeOutput(const std::string& filename)
{
  ...
  for(int i = 0; i <= grid.maxXIndex; i++)
    {
      const auto rhoV2 = grid.quantities[i][EquationsEulerPassiveScalar::XMOM] *
                         grid.quantities[i][EquationsEulerPassiveScalar::XMOM] / grid.quantities[i][EquationsEulerPassiveScalar::DENS];
      const auto p = (static_cast<EquationsEulerPassiveScalar*>(&equations))
                         ->getPressure(grid.quantities[i][EquationsEulerPassiveScalar::ENERGY], rhoV2);
      ...
      outputFile << grid.getX(i) << " " << grid.quantities[i][EquationsEulerPassiveScalar::DENS] << " "
                 << grid.quantities[i][EquationsEulerPassiveScalar::XMOM] / grid.quantities[i][EquationsEulerPassiveScalar::DENS]
                 << " " << p << " " << grid.quantities[i][EquationsEulerPassiveScalar::PASS] << std::endl;
    }
